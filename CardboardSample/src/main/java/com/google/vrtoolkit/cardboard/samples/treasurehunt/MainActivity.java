...
public class MainActivity extends CardboardActivity implements CardboardView.StereoRenderer {
  ...
  public void onSurfaceCreated(EGLConfig config) {
    ...
    /*Matrix.setIdentityM(modelCubes[0], 0);
    Matrix.translateM(modelCubes[0], 0, 0, 0, -objectDistance);
      Matrix.setIdentityM(modelCubes[1], 0);
      Matrix.translateM(modelCubes[1], 0, 10, 0, -objectDistance);*/
      for (int i = 0; i < modelCubes.length; i++)
      {
   ...
   */
  public void drawCube(int objNo) {
    GLES20.glUseProgram(cubeProgram);
    ...
    //GLES20.glUniformMatrix4fv(cubeModelParam, 1, false, modelCube, 0);
      ...
      for (int i = 0; i < modelCubes.length; i++)
      {
          GLES20.glUniformMatrix4fv(cubeModelParam, 1, false, modelCubes[i], 0);
      }
    ...
    GLES20.glVertexAttribPointer(cubeColorParam, 4, GLES20.GL_FLOAT, false, 0,
        isLookingAtObject(objNo) ? cubeFoundColors : cubeColors);
  ...
  public void onCardboardTrigger() {
    Log.i(TAG, "onCardboardTrigger");
      ...
      for (int i = 0; i < modelCubes.length; i++)
      {
          if (isLookingAtObject(i)) {
              ...
              if(i==1) {
                  Runnable runnable = new Runnable() {
                      @Override
                      public void run() {
                          Intent intent = new Intent(MainActivity.this, DeuteranopiaActivity.class);
                          startActivity(intent);
                      }
                  };
                  new Thread(runnable).start();
              ...
              if(i==2) {
                  Runnable runnable = new Runnable() {
                      @Override
                      public void run() {
                          Intent intent = new Intent(MainActivity.this, TritanopiaActivity.class);
                          startActivity(intent);
                      }
                  };
                  new Thread(runnable).start();
        ...
        String output = "app does not exist";
        if (isIntentSafe) output = "app exists";
        Log.e("tag", output);
        if (isIntentSafe) {
            Log.e("tag", "another activity should open");
            startActivity(intent);
        }*/
  ...
  private void hideObject() {
   ...
   /* Matrix.multiplyMV(posVec, 0, rotationMatrix, 0, modelCube, 12);
      Matrix.multiplyMV(posVec, 0, rotationMatrix, 0, modelCube2, 12);*/
      for (int i = 0; i < modelCubes.length; i++)
      {
          Matrix.multiplyMV(posVec, 0, rotationMatrix, 0, modelCubes[i], 12);
      }
    ...
    Matrix.translateM(modelCube, 0, posVec[0], newY, posVec[2]);
      ...
      Matrix.translateM(modelCube2, 0, posVec[0] + 5, newY, posVec[2] + 5);*/

      for (int i = 0; i < modelCubes.length; i++)
      {
          Matrix.setIdentityM(modelCubes[i], 0);
          Matrix.translateM(modelCubes[i], 0, posVec[0] + 5, newY, posVec[2] + 5);
      }
  }
   ...
   */
  private boolean isLookingAtObject(int objectNumber) {
    float[] initVec = { 0, 0, 0, 1.0f };
    float[] objPositionVec = new float[4];

    // Convert object space to camera space. Use the headView from onNewFrame.
    Matrix.multiplyMM(modelView, 0, headView, 0, modelCubes[objectNumber], 0);
